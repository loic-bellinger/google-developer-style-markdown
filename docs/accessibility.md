...
- In HTML, use [semantic
  tagging](https://developers.google.com/style/semantic-tagging). For example, use the `em` element only to indicate emphasis, not to indicate italics.
- In HTML, prefer [native
...
[Text associated with images](https://developers.google.com/style/images#text-associated-with-images).
...
[Menu bar](https://developers.google.com/style/ui-elements#term-menus).
...
- If your tables include both row and column headings, then mark heading cells with the [`scope`
  attribute](https://www.w3.org/WAI/tutorials/tables/two-headers/).
- If your tables have more than one row containing column headings, then use the [`headers`
...
- Pick colors that respect [accessible color contrast
...
- Don't use directional language to orient the reader, such as *above*, *below*,
  ...
  *preceding*, or *following*.
  ...
  If a [UI element](https://developers.google.com/style/ui-elements) is hard to find,
  [provide a screenshot](https://developers.google.com/style/images).
