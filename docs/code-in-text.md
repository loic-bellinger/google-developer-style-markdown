...
| [HTTP status codes](https://developers.google.com/style/code-in-text#statuscodes) | The HTTP `500 Internal Server Error` status code indicates that the server encountered an unexpected condition that prevented it from fulfilling the request. <br /> |
...
| [Method and function names](https://developers.google.com/style/code-in-text#methods) | The `ST_GEOPOINT` function uses the longitude and latitude of the Colosseum in Rome. To fetch the status of the job, call the `get_job_status` method. |
...
If a
[UI element](https://developers.google.com/style/ui-elements#formatting)
meets the
[requirements for code font](https://developers.google.com/style/code-in-text#code),
...
In particular, call it a *status code* instead of a *response code* or *error code*, and put the number and the name in code font.
...
In particular, use *<var translate="no">N</var>xx* (with a specific digit in place of
<var translate="no">N</var>) to indicate *anything in the <var translate="no">N</var>00-<var translate="no">N</var>99 range*, and put the status code number in code font even if you're leaving
out the code's name.
