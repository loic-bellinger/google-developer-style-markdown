...
![Bounded contexts are applied to an application.](https://cloud.google.com/architecture/images/microservices-architecture-refactoring-monoliths-bounded-contexts.svg) **Figure 1**. Application capabilities are separated into bounded contexts that migrate to services. In figure 1, the ecommerce application's capabilities are separated into
...
- Figure numbers are optional. If you use figure numbers, use the form "\<b\>Figure
  <var translate="no">NUMBER</var>.\</b\> <var translate="no">DESCRIPTION</var>."
...
- Currently, only an additional `2x` image is necessary, but someday screen may increase further. So the `srcset` attribute supports further alternative sizes, each specified by the appropriate multiplier, such as `3x` or `4x`.
...
For more information, see the HTML specification for the `https://html.spec.whatwg.org/multipage/embedded-content.html#the-img-element`
element.
